...
let coords={'australia':[83,72],'united states':[17,35],'usa':[17,35],'canada':[15,24],'united kingdom':[46,28],'uk':[46,28],'france':[48,34],'germany':[51,31],'italy':[51,38],'spain':[45,39],'netherlands':[49,29],'singapore':[76,58],'japan':[84,36],'china':[74,37],'india':[68,48],'uae':[61,47],'united arab emirates':[61,47],'maldives':[66,58],'argentina':[29,76],'brazil':[31,66],'south africa':[51,74],'new zealand':[91,78],'switzerland':[50,34],'monaco':[49,37]};let markers=$('#mapMarkers');markers.innerHTML='';places.slice(0,18).forEach((x,i)=>{let k=x.country.toLowerCase(),xy=coords[k];if(!xy){let h=[...k].reduce((a,c)=>a+c.charCodeAt(0),0);xy=[12+(h*7)%76,25+(h*11)%52]}let m=document.createElement('i');m.className='map-marker';m.style.left=xy[0]+'%';m.style.top=xy[1]+'%';m.dataset.label=`${x.city||x.country} • ${x.r.name.replace('NEXUS ','')}`;markers.appendChild(m)});$('#mapCount').textContent=`${places.length} DESTINATION${places.length===1?'':'S'}`;$('#mapLegend').innerHTML=places.length?places.slice(0,10).map(x=>`<span>${x.r.icon} ${esc(x.city||x.country)} · ${esc(x.country)}</span>`).join(''):'<span>No destinations configured yet.</span>';
