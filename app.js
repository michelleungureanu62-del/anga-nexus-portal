...
function divFmt(n,d=0){return Number(n).toLocaleString('en-US',{minimumFractionDigits:d,maximumFractionDigits:d})}
function renderDividendGrid(tierIndex){
  const D=C.dividends;if(!D||tierIndex<0)return;
  const rates=D.rates[tierIndex],g=$('#dividendGrid');if(!g)return;
...
function renderDividendCenter(){
  ...
  $('#divTier').textContent=`TIER ${tier.tier}`;$('#divTierName').textContent=tier.name;$('#divBalance').textContent=fmt(currentBalance);
  $('#divRange').textContent=`${fmt(tier.min)} – ${tier.max===null?'∞':fmt(tier.max)} ANGA`;
...
function dash(b){currentBalance=b;$('#balance').textContent=fmt(b);if(!tier){$('#tier').textContent='NOT ELIGIBLE';$('#tierName').textContent='Minimum 90 ANGA';$('#next').textContent=`${fmt(Math.max(0,90-b))} ANGA to Foundation`;$('#bar').style.width=`${Math.min(100,b/90*100)}%`;return}$('#tier').textContent=`TIER ${tier.tier}`;$('#tierName').textContent=tier.name;let n=C.tiers[tier.tier];if(n){$('#next').textContent=`${fmt(Math.max(0,n.min-b))} ANGA to ${n.name}`;$('#bar').style.width=`${Math.min(100,(b-tier.min)/(n.min-tier.min)*100)}%`}else{$('#next').textContent='HIGHEST ANGA LEVEL';$('#bar').style.width='100%'}}async function lookup(){let a=$('#address').value.trim().toUpperCase();if(!/^G[A-Z2-7]{55}$/.test(a)){status('Enter a valid Stellar public G-address.');return}status('Reading Stellar ledger…');try{let r=await fetch(`https://horizon.stellar.org/accounts/${a}`);if(!r.ok)throw Error('Stellar account not found or unavailable.');let j=await r.json(),x=j.balances.find(b=>b.asset_code===C.asset.code&&b.asset_issuer===C.asset.issuer),b=Number(x?.balance||0);acct=a;tier=tierFor(b);await load();dash(b);$('#dash').classList.remove('hidden');if(tier){$('#rewards').classList.remove('hidden');rewards();renderDividendCenter();status(`Connected • ${tier.name} detected.`)}else{$('#rewards').classList.add('hidden');$('#dividends')?.classList.add('hidden');status(`Connected • ${fmt(b)} ANGA detected. Tier 1 starts at 90 ANGA.`)}localStorage.setItem('anga:last',a)}catch(e){status(e.message)}}function status(t){$('#status').textContent=t}function esc(s=''){return String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function openR(r){
  ...
  // V1.1.3 — every remaining reward category uses repeatable allocations.
  // This keeps old single-configuration saves compatible while allowing investors
  // to add as many separate reward instructions as their entitlement requires.
  if(preferenceRewards.includes(r.id)){
    const singular={vehicles:'VEHICLE',supercars:'SUPERCAR',jets:'JET',helicopters:'HELICOPTER',yachts:'YACHT',ships:'SHIP',healingunits:'HEALING UNIT',healingcentres:'HEALING CENTRE',energy:'ENERGY SYSTEM',mining:'MINING UNIT'}[r.id]||'REWARD';
    const defaultUse=r.modes[2]||r.modes[1]||r.modes[0];
    let allocations=Array.isArray(o.allocations)&&o.allocations.length?o.allocations.map(a=>({...a})):[{
      quantity:Object.values(o.alloc||{}).reduce((a,b)=>a+Math.max(0,Number(b||0)),0),
      use:Object.entries(o.alloc||{}).sort((a,b)=>Number(b[1])-Number(a[1]))[0]?.[0]||defaultUse,
      preference:o.preference||'',buildStyle:o.buildStyle||'',features:o.features||'',country:o.country||'',region:o.region||'',city:o.city||'',instructions:o.instructions||'',mine:o.mine||'Gold'
    }];
    const hero=rewardPhotos[r.id]?`<div class="reward-single-hero"><img src="${rewardPhotos[r.id]}" alt="${r.name}"></div>`:'';
    $('#body').innerHTML=`<span>${r.icon} NEXUS REWARD MANAGEMENT</span><h2>${r.name}</h2>${hero}${statusTimeline(o.status||'REQUESTED')}<div class="house-total-line"><span>${singular} ALLOCATIONS</span><strong>${fmt(q)} AVAILABLE</strong></div><p class="house-intro">Configure your ${r.name} in separate allocations. Add another allocation whenever you need different models, features, destinations or instructions.</p><div class="house-allocation-summary"><div><small>ALLOCATED</small><b id="houseAllocated">0</b></div><div><small>REMAINING</small><b id="houseRemaining">${fmt(q)}</b></div><div><small>ALLOCATIONS</small><b id="houseAllocationCount">0</b></div></div><div class="house-progress"><i id="houseProgress"></i></div><div id="houseAllocations"></div><button type="button" id="addHouseAllocation" class="btn ghost house-add">+ ADD ${singular} ALLOCATION</button><div class=estimate><small>MONTHLY REVENUE</small><b id=est>$0 – $0+ / month</b><p>Based on working and NEXUS Managed quantities across all allocations.</p></div><p id="houseError" class="house-error"></p><button id=saveR class="btn save">SAVE ${singular} CONFIGURATION</button>`;
    $('#modal').classList.remove('hidden');
    const holder=$('#houseAllocations');
    function card(a,i){
      const mine=r.id==='mining'?`<div class="field"><label>WHAT WOULD YOU LIKE TO MINE?</label><select class="ha-mine">${['Gold','Silver','Copper','Lithium','Nickel','Iron Ore','Uranium','Rare Earths','Other'].map(x=>`<option ${a.mine===x?'selected':''}>${x}</option>`).join('')}</select><small>Multiplier: ${r.multipliers[tier.tier-1]}</small></div>`:'';
      return `<article class="house-allocation-card" data-i="${i}"><div class="house-allocation-head"><div><small>${singular} ALLOCATION</small><h3>Allocation ${String(i+1).padStart(2,'0')}</h3></div>${allocations.length>1?'<button type="button" class="remove-house-allocation">REMOVE</button>':''}</div><div class="house-control-column reward-repeat-controls"><section class="house-config-section"><div class="house-section-label"><small>01</small><div><b>ALLOCATION</b><span>Choose quantity and intended use.</span></div></div><div class="house-fields"><div class="field"><label>QUANTITY</label><input class="ha-qty" type="number" min="0" max="${q}" step="1" value="${Math.max(0,Number(a.quantity||0))}"></div><div class="field"><label>USE</label><select class="ha-use">${r.modes.map(x=>`<option ${a.use===x?'selected':''}>${x}</option>`).join('')}</select></div></div></section><section class="house-config-section"><div class="house-section-label"><small>02</small><div><b>BUILD MY REWARD</b><span>Specify the model, style and features for this allocation.</span></div></div><div class="house-fields"><div class="field"><label>STYLE / CATEGORY / MODEL</label><input class="ha-preference" value="${escAttr(a.preference||'')}" placeholder="Model, category or preference"></div><div class="field"><label>KEY FEATURES</label><input class="ha-features" value="${escAttr(a.features||'')}" placeholder="Size, capacity, amenities, specifications..."></div>${mine}</div></section><section class="house-config-section"><div class="house-section-label"><small>03</small><div><b>DESTINATION</b><span>Give this allocation its own destination and instructions.</span></div></div><div class="house-fields"><div class="field"><label>COUNTRY</label><input class="ha-country" value="${escAttr(a.country||'')}" placeholder="Any country"></div><div class="field"><label>STATE / REGION</label><input class="ha-region" value="${escAttr(a.region||'')}"></div><div class="field full"><label>CITY</label><input class="ha-city" value="${escAttr(a.city||'')}" placeholder="Any city"></div><div class="field full"><label>DESTINATION / INSTRUCTIONS</label><textarea class="ha-instructions" placeholder="Any other details you need can be specified here">${escAttr(a.instructions||'')}</textarea></div></div></section></div></article>`;
    }
    function calc(){
      const total=allocations.reduce((s,a)=>s+Math.max(0,Number(a.quantity||0)),0), remaining=Math.max(0,q-total);
      const working=allocations.reduce((s,a)=>s+((a.use===r.modes[0])?0:Math.max(0,Number(a.quantity||0))),0);
    ...
    function render(){
      holder.innerHTML=allocations.map(card).join('');
      holder.querySelectorAll('.house-allocation-card').forEach((el,i)=>{
        const bind=(sel,key,num=false)=>{const x=el.querySelector(sel);if(!x)return;x.oninput=()=>{allocations[i][key]=num?Math.max(0,Math.floor(Number(x.value||0))):x.value;calc()};x.onchange=x.oninput};
        bind('.ha-qty','quantity',true);bind('.ha-use','use');bind('.ha-preference','preference');bind('.ha-features','features');bind('.ha-country','country');bind('.ha-region','region');bind('.ha-city','city');bind('.ha-instructions','instructions');bind('.ha-mine','mine');
        const rem=el.querySelector('.remove-house-allocation');if(rem)rem.onclick=()=>{allocations.splice(i,1);render()};
      });
      calc();
    }
...
[...document.querySelectorAll('[data-div-period]')].forEach(b=>b.onclick=()=>{[...document.querySelectorAll('[data-div-period]')].forEach(x=>x.classList.remove('active'));b.classList.add('active');divPeriod=Number(b.dataset.divPeriod);renderDividendGrid(divPreviewTier===null?(tier?.tier||1)-1:divPreviewTier)});
